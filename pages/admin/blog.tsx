...
const postVazio: Omit<Post, 'id' | 'slug'> = {
  titulo: '',
  resumo: '',
  conteudo: '',
  imagem: '',
...
export default function AdminBlog() {
  ...
  const [form, setForm] = useState(postVazio)
  ...
  // ── autenticação ──────────────────────────────────────────────────
  useEffect(() => {
    const ok = sessionStorage.getItem('baldas-auth') === 'ok'
  ...
  const carregarPosts = useCallback(() => {
    try {
      ...
      const lista: Post[] = raw ? JSON.parse(raw) : []
      setPosts(lista.sort((a, b) => b.data.localeCompare(a.data)))
    } catch {
      setPosts([])
    }
  }, [])
  ...
  // ── salvar posts ──────────────────────────────────────────────────
  const salvarPosts = (lista: Post[]) => {
    localStorage.setItem('baldas-blog-posts', JSON.stringify(lista))
    setPosts(lista.sort((a, b) => b.data.localeCompare(a.data)))
  }
  ...
  const novoPost = () => {
    setEditando(null)
    setForm({ ...postVazio, data: new Date().toISOString().split('T')[0] })
    setAba('editor')
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editando) {
      const atualizado = lista.map(p =>
        p.id === editando.id ? { ...p, ...form, slug } : p
      )
      salvarPosts(atualizado)
    } else {
      const novo: Post = {
  ...
  const deletarPost = (id: string) => {
    const raw = localStorage.getItem('baldas-blog-posts')
    const lista: Post[] = raw ? JSON.parse(raw) : []
    salvarPosts(lista.filter(p => p.id !== id))
      ...
      <>
        ...
        <div className="min-h-screen flex items-center justify-center p-4" style={{ backgroundColor: '#F8FAFB' }}>
          <div className="w-full max-w-sm">
            ...
            >
              <label className="block text-sm font-medium mb-2" style={{ color: '#0A3244' }}>
                Senha de acesso
              </label>
              <input
                type="password"
                ...
                style={{
                  borderColor: erroSenha ? '#ef4444' : '#D6E8EF',
                  color: '#0F1923',
                }}
                autoFocus
              />
              {erroSenha && (
                <p className="text-xs text-red-500 mb-4">Senha incorreta. Tente novamente.</p>
              )}
              <button
                type="submit"
            ...
            <div className="text-center mt-4">
              <Link href="/" className="text-xs" style={{ color: '#4A6572' }}>
                ← Voltar ao site
              </Link>
            </div>
          </div>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-14">
            ...
            <div className="flex items-center gap-4">
              <Link href="/blog" target="_blank" className="text-xs" style={{ color: '#A8CDD9' }}>
                Ver blog ↗
              </Link>
              <Link href="/" className="text-xs" style={{ color: '#A8CDD9' }}>
                Sair
              </Link>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between mb-6">
            <div className="flex gap-2">
              <button
                ...
                style={{
                  backgroundColor: aba === 'lista' ? '#0A3244' : '#fff',
                  color: aba === 'lista' ? '#fff' : '#4A6572',
              ...
              <button
                ...
                className="px-4 py-2 rounded-lg text-sm font-medium transition-colors"
                style={{
                  backgroundColor: aba === 'editor' ? '#0A3244' : '#fff',
                  color: aba === 'editor' ? '#fff' : '#4A6572',
                  border: '1px solid #D6E8EF',
                }}
              >
            ...
            <div className="space-y-3">
              {posts.length === 0 && (
                <div
                  ...
                  style={{ backgroundColor: '#fff', border: '1px dashed #D6E8EF' }}
                >
              ...
              {posts.map(post => (
                ...
                >
                  ...
                  <button
                    ...
                    style={{
                      backgroundColor: post.publicado ? '#D1FAE5' : '#FEF3C7',
                      color: post.publicado ? '#065F46' : '#92400E',
                    }}
                  >
                  ...
                  <div className="flex gap-2 shrink-0">
                    <button
                      ...
                      style={{ backgroundColor: '#F0F5F7', color: '#0A3244' }}
                    >
                      Editar
                    </button>
            ...
            >
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
                ...
                <div className="lg:col-span-2">
                  <label className="block text-sm font-medium mb-1.5" style={{ color: '#0A3244' }}>
                    Título *
                  </label>
                  <input
                    type="text"
                  ...
                  />
                  {form.titulo && (
                    <p className="text-xs mt-1" style={{ color: '#4A6572' }}>
                      URL: /blog/{gerarSlug(form.titulo)}
                    </p>
                  )}
                </div>
              ...
              <div className="mb-6">
                ...
                <textarea
                  ...
                  placeholder="Escreva o conteúdo completo do artigo aqui. Deixe uma linha em branco entre os parágrafos."
                  rows={16}
                  required
                ...
                />
                <p className="text-xs mt-1" style={{ color: '#4A6572' }}>
                  Dica: deixe uma linha em branco entre parágrafos para criar espaçamento.
                </p>
              </div>
